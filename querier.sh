...
function query {
  storage_root="${1}"
  expression="${2}"
  tools/querier/querier -storage.root="${storage_root}" -alsologtostderr -range=48h -expression="${expression}" -expectedElements=1 -resolution=1m -numWarmupQueries=0 -numQueries=1 2>&1
}
